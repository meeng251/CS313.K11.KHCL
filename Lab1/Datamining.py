...
#Option 4:
    ...
    solution = int(input("Please choose normalization solution: (1)Min-Max \n (2)Z-score"))
    if solution == 1:
        TypeOfData = int(intput("Please choose type of input: (1)Có dữ liệu thiếu cho cả dữ liệu nominal và numeric, với số lượng mẫu là 20 \n (2)Dữ liệu đầy đủ với số lượng mẫu là 20. \n (3)Dữ liệu đầy đủ với số lượng mẫu lớn hơn 100."))
        if TypeOfData ==1:
            df= pd.read_csv()
            data = df.values
            ...
            scaled_data.to_excel(excel_writer="D:/trang download/Datamining/Datamining/result.xlsx")
        if TypeOfData ==2:
            df= pd.read_csv()
            data = df.values
    ...
    if solution == 2:
        ...
        if TypeOfData ==1:
            ...
            result.to_excel(excel_writer="D:/trang download/Datamining/Datamining/result.xlsx")
        if TypeOfData ==1:
            df=pd.read_csv()
            result =stats.zscore(df)
